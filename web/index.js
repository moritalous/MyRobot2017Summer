...
const shadows = AWSIoTData.thingShadow({
      ...
      //
      clientId: 'clientId-' + (Math.floor((Math.random() * 100000) + 1)),
      //
...
shadows.on('delta', function (name, stateObject) {
      ...
      console.log(stateObject.state.face.type);

      changeFace(stateObject.state.face.type);
});
...
window.shadowConnectHandler = function () {
      ...
      //
      setTimeout(function () {
            var opClientToken = shadows.get('testThing1');
            if (opClientToken === null) {
                  console.log('operation in progress');
            }
            opClientToken = shadows.get('testThing1');
            if (opClientToken === null) {
                  console.log('operation in progress');
            }
      }, 3000);
